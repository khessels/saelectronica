/* ============================================================
   ...
   - data-lang: "es" (default) or "en" — which language this page is in.
     Spanish and English pages can have entirely different filenames/slugs
     (e.g. reparacion.html <-> repair-en.html), so every nav item's href
     is given explicitly for each language below — nothing is derived
     from a filename pattern.
   ============================================================ */
(function () {
  ...
  var items = [
    { id: 'inicio', href: { es: 'index.html', en: 'index-en.html' }, label: { es: 'Inicio', en: 'Home' } },
    { id: 'reparacion', href: { es: 'reparacion.html', en: 'repair.html' }, label: { es: 'Reparación', en: 'Repair' } },
    { id: 'diseno', label: { es: 'Diseño y Prototipo', en: 'Design & Prototyping' }, children: [
        { id: 'iot', href: { es: 'diseno-iot-y-iiot.html', en: 'iot-iiot-design.html' }, label: { es: 'Diseño IoT\\IIoT', en: 'IoT\\IIoT Design' } },
        { id: 'pcb', href: { es: 'diseno-circuitos-y-placas-pcb.html#pcb-circuitos', en: 'pcb-circuit-design.html#pcb-circuitos' }, label: { es: 'Diseño PCB / Circuitos', en: 'PCB / Circuit Design' } }
      ]
    },
    { id: 'firmware', href: { es: 'firmware.html', en: 'firmware-en.html' }, label: { es: 'Firmware', en: 'Firmware' } },
    { id: 'proyectos', href: { es: 'proyectos.html', en: 'projects.html' }, label: { es: 'Proyectos', en: 'Projects' } },
    { id: 'contacto', href: { es: 'contacto.html', en: 'contact.html' }, label: { es: 'Contacto', en: 'Contact' } }
  ];
  ...
  var linksHTML = items.map(function (item) {
    if (item.children) {
      ...
      var childrenHTML = item.children.map(function (child) {
        ...
        return '<li><a href="' + child.href[lang] + '"' + activeAttr + '>' + child.label[lang] + '</a></li>';
      }).join('');
    ...
    return '<li><a href="' + item.href[lang] + '"' + activeAttr + '>' + item.label[lang] + '</a></li>';
  }).join('');
  ...
  /* Language switcher: links to the exact counterpart page (same page, other
     language). Every page must set data-alt-href on <body> since Spanish and
     English filenames no longer share a predictable pattern. */
  var altHref = document.body.getAttribute('data-alt-href') || (lang === 'en' ? 'index.html' : 'index-en.html');
  ...
  var html =
    '<header>' +
      '<nav class="wrap">' +
        '<a href="' + items[0].href[lang] + '" class="logo"><img src="logo-mark-full.png" alt="Electr\u00F3nica Santa Ana" class="logo-img"></a>' +
        '<div class="nav-right">' +
          langSwitch +
          facebookIcon +
